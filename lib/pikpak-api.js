...
(function (root) {
  ...
  class PikPakClient {
    ...
    async resolveMediaStreams(shareId, fileId) {
      ...
      const isNonNative = nonNativeContainers.has(fileExt) || /video\/(x-msvideo|avi|msvideo|x-ms-wmv|x-flv)/i.test(info.mime_type || "");

      const origSize = parseInt(info.size, 10) || 0;
      const durSec = parseInt(info.params?.duration || info.medias?.[0]?.video?.duration || 0, 10);

      // Luôn tìm hoặc khởi tạo luồng Original (File gốc nguyên bản)
      const origInMedias = medias.find((m) => m.media_name === "original" && m.link?.url && !m.link.url.includes("fid=&"));
      ...
      if (origUrl) {
        ...
        streams.push({
          ...
          compressionRatio: 0,
          mimeType: info.mime_type || (isNonNative ? `video/${fileExt}` : "video/mp4"),
        });
      }
      ...
      for (const m of medias) {
        if (m.link?.url && !m.link.url.includes("fid=&")) {
          ...
          if (m.link.url === origUrl || m.media_name === "original") continue;
          ...
          streams.push({
            quality: m.media_name || "Transcoded",
