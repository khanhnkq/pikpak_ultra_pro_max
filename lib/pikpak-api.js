...
(function (root) {
  ...
  class PikPakClient {
    ...
    setupPlatform(platform = "android") {
      this.platform = platform;
      if (platform === "android") {
        this.clientId = C.ANDROID_CLIENT_ID || "YNxT9w7GMdWvEOKa";
        this.clientVersion = C.ANDROID_CLIENT_VERSION || "1.53.2";
        this.packageName = C.ANDROID_PACKAGE_NAME || "com.pikcloud.pikpak";
        this.algorithms = C.ANDROID_ALGORITHMS || [];
        this.userAgent = C.ANDROID_UA || "PikPak/1.53.2 (Linux; U; Android 14; M2004j7ac; Build/UKQ1.231108.001)";
      } else {
        this.platform = "web";
        this.clientId = C.WEB_CLIENT_ID || "YUMx5nI8ZU8Ap8pm";
        this.clientVersion = C.WEB_CLIENT_VERSION || "2.0.0";
        this.packageName = C.WEB_PACKAGE_NAME || "mypikpak.com";
        this.algorithms = C.WEB_ALGORITHMS || [];
        this.userAgent = C.DEFAULT_UA;
      }
    }
    ...
    async refreshCaptcha(action = "GET:/drive/v1/share/detail") {
      ...
      for (let attempt = 0; attempt < this.userHosts.length; attempt++) {
        ...
        try {
          ...
          if (attempt === this.userHosts.length - 1 && this.platform === "android") {
            console.warn("[PikPakClient] Android captcha init failed, falling back to Web mode...");
            this.setupPlatform("web");
            return this.refreshCaptcha(action);
          }
    ...
    async resolveMediaStreams(shareId, fileId) {
      ...
      // 4. Primary URL mặc định luôn luôn là Original
      let primaryUrl = "";
      ...
      if (isNonNative && bestTranscoded) {
        ...
        primaryUrl = origStream?.url || origUrl || streams[0]?.url || "";
      }
