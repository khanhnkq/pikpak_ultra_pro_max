    ...
    async listFolderFiles(shareId, passCode = "", parentId = "") {
      ...
      const tryResolveAsDirectFile = async (targetId) => {
        try {
          ...
          const fileInfo = raw?.file_info || raw;
          if (fileInfo?.id && fileInfo.kind !== "drive#folder") {
            console.log(`[PikPakClient] ✅ ${targetId} là file trực tiếp: ${fileInfo.name}`);
            ...
            const isImage = (C.IMAGE_EXTENSIONS || []).includes(ext) || (fileInfo.mime_type || "").startsWith("image/");
            const singleMedia = {
              id: fileInfo.id,
              name: fileInfo.name,
            ...
            try {
              ...
              if (rootRes && rootRes.videos.length > 0) {
                if (!rootRes.videos.some((v) => v.id === fileInfo.id)) {
                  if (isVideo) rootRes.videos.unshift(singleMedia);
                  if (rootRes.allFiles) rootRes.allFiles.unshift(singleMedia);
                }
                rootRes.targetFileId = fileInfo.id;
                return rootRes;
              }
            } catch (_) {}

            return {
              videos: isVideo ? [singleMedia] : [],
              subfolders: [],
              allFiles: [singleMedia],
              mediaFiles: (isVideo || isImage) ? [singleMedia] : [],
              currentParentId: "",
